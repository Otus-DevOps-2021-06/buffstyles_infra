...
### Основное задание:
 ...
 - Созданы плейбуки с разными подходами:

 Один плейбук - один сценарий;

 Один плейбук - несколько сценариев;

 Несколько плейбуков с один плейбуком для деплоя.

 - Изменены провижены образов Packer со скриптов на плейбуки
 - Выполнен билд образов с использованием нового
провижинера.
 - На основе созданных app и db образов запущено stage окружение.
 - С помощью плейбука `site.yml` окружение конфигурируется, а приложение деплоится и работает.

### Дополнительное задание 1:
 - При использовании в `ansible.cfg` скрипта `nechitaemoe_nechto.py` для генерации динамического инвентаря и динамической переменной `db_host` в плейбуке `app.yml`, с помощью плейбука `site.yml` можно произвести деплой приложения сразу после поднятия stage инфраструктуры.
