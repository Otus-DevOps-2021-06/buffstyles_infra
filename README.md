...
### Основное задание:
 ...
 - Описано два окружения - `stage` и `prod`
 - Использована коммьюнити роль nginx.
 - Использован `Ansible Vault` для наших окружений.

 ### Дополнительное задание 1:

  - Скрипт динамической инвентаризации добавлен `environments` каждого из окружений.
...
### Основное задание:
 ...
 - Произведена локальная разработка ролей и плейбуков при помощи `Vagrant`,
доработка ролей для провижининга в `Vagrant`.
 - Произведено тестирование ролей при помощи `Molecule` и
`Testinfra`.
 - Сбор образов пакером переключен на
использование ролей.

### Дополнительное задание 1:

 - Дополнена конфигурация Vagrant для
корректной работы проксирования приложения
с помощью nginx.
